...
function getMenu() {
  return [
    [{ text: 'Play Now! 🎡', web_app: { url: 'https://c93a-2402-800-61ef-b75b-9430-2d74-f500-944a.ngrok-free.app/' }}],
...
bot.start((ctx) => {
  ...
  if (refId) {
    ...
    axios.post(`${process.env.REACT_APP_API_URL}referral`, {
...
bot.command('referral', (ctx) => {
  let referrals = 0;
  axios.get(`${process.env.REACT_APP_API_URL}user/${ctx.from.id}`)
  .then((response) => {
...
bot.action('show_profile', (ctx) => {
  ...
  // get user rank
  axios.get(`${process.env.REACT_APP_API_URL}user/${ctx.from.id}/get-rank`)
    .then((response) => {
