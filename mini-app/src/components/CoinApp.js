...
export default function CoinApp(props) {
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      setMiningInfo(prevMiningInfo => {
        ...
        if (userData.id)  axios.get(`${API_URL}user/${userData.id}`)
  ...
  useEffect(() => {
    const req = async () => {
      await axios.post(`${API_URL}user/${userId}/add-point`, {
  ...
  useEffect(() => {
    const req = async () => {
      ...
      await axios.get(`${API_URL}user/${userId}/get-rank`)
      .then(response => {
      ...
      await axios.get(`${API_URL}leaderboard`)
      .then(response => {
